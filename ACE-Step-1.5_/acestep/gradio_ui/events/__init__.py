...
def setup_event_handlers(demo, dit_handler, llm_handler, dataset_handler, dataset_section, generation_section, results_section):
    ...
    generation_section["lora_scale_slider"].change(
        ...
        outputs=[generation_section["lora_status"]],
        api_name="set_lora_scale"
    )
    
    # LoRA status query (API-only, returns JSON string with trigger_tag etc.)
    _lora_status_btn = gr.Button(visible=False)
    _lora_status_output = gr.JSON(visible=False)
    _lora_status_btn.click(
        fn=dit_handler.get_lora_status,
        outputs=[_lora_status_output],
        api_name="get_lora_status"
    )
    
    # LoRA tag injection mode (API-only, changes prepend/append/off at runtime)
    _tag_position_input = gr.Textbox(visible=False)
    _tag_position_output = gr.Textbox(visible=False)
    _tag_position_input.change(
        fn=dit_handler.set_tag_position,
        inputs=[_tag_position_input],
        outputs=[_tag_position_output],
        api_name="set_tag_position"
    )
