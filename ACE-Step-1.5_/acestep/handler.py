...
class AceStepHandler:
    ...
    def _check_lora_base_model_compatibility(self, lora_path: str) -> Tuple[bool, str]:
        ...
        if is_turbo_lora != is_turbo_current:
            return False, f"⚠️ INCOMPATIBLE: LoRA trained on '{base_model_name}' but current model is '{current_model_name}'."
        if base_model_name.lower() != current_model_name.lower():
            return False, f"⚠️ Model mismatch: LoRA trained on '{base_model_name}', current is '{current_model_name}'."
        return True, ""
    
    def _count_lora_layers(self) -> Tuple[int, int]:
        """Count the number of LoRA layers and total trainable parameters."""
        num_lora_layers = 0
        num_trainable_params = 0
        try:
            for name, module in self.model.decoder.named_modules():
                module_type = type(module).__name__
                if 'Lora' in module_type or 'lora' in module_type:
                    num_lora_layers += 1
                    for param in module.parameters():
                        num_trainable_params += param.numel()
        except Exception as e:
            logger.warning(f"Could not count LoRA layers: {e}")
        return num_lora_layers, num_trainable_params
    
    def set_tag_position(self, tag_position: str) -> str:
        """Change trigger tag injection mode (prepend/append/off) at runtime."""
        valid = ['prepend', 'append', 'off']
        if tag_position not in valid:
            return f"❌ tag_position must be one of: {', '.join(valid)}"
        self._current_lora_metadata['tag_position'] = tag_position
        trigger_tag = self._current_lora_metadata.get('trigger_tag', '')
        if tag_position == 'off':
            return f"🏷️ Tag auto-injection disabled (trigger: '{trigger_tag}')"
        return f"🏷️ Tag injection: '{trigger_tag}' → {tag_position}"
    ...
    def load_lora(self, lora_path: str) -> str:
        ...
        # Robust path normalization (handles Windows paths with backslashes, quotes, etc.)
        lora_path = lora_path.strip().strip('"').strip("'")
        lora_path = os.path.normpath(lora_path)  # normalize slashes and resolve ..
        
        logger.info(f"LoRA path after normalization: {lora_path}")
        ...
        if not os.path.exists(config_file):
            # Auto-search: maybe the adapter_config.json is in a subdirectory
            found = False
            for root, dirs, files in os.walk(lora_path):
                if "adapter_config.json" in files:
                    lora_path = root
                    config_file = os.path.join(root, "adapter_config.json")
                    found = True
                    logger.info(f"Found adapter_config.json in subdirectory: {lora_path}")
                    break
                # Don't recurse too deep
                if root.count(os.sep) - lora_path.count(os.sep) > 2:
                    break
            if not found:
        ...
        try:
            ...
            if self._base_decoder is None:
                ...
                # Store backup on CPU to save ~10 GB VRAM
                self._base_decoder = self._base_decoder.to("cpu")
                _gc.collect()
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
                logger.info("Base decoder backed up (stored on CPU to save VRAM)")
            else:
                # Restore base decoder before loading new LoRA (copy from CPU back to GPU)
            ...
            self._current_lora_metadata = self._read_lora_metadata(lora_path)
            
            # Verify LoRA layers were applied
            num_lora_layers, num_lora_params = self._count_lora_layers()
            
            # Build detailed status message
            status_parts = []
            lora_name = os.path.basename(lora_path)
            rank = self._current_lora_metadata.get("rank", "?")
            alpha = self._current_lora_metadata.get("alpha", "?")
            trigger_tag = self._current_lora_metadata.get("trigger_tag", "")
            
            status_parts.append(f"✅ LoRA '{lora_name}' loaded (r={rank}, α={alpha})")
            
            if num_lora_layers > 0:
                params_str = f"{num_lora_params/1e6:.1f}M" if num_lora_params > 1e6 else f"{num_lora_params/1e3:.0f}K"
                status_parts.append(f"📊 {num_lora_layers} layers, {params_str} params")
            else:
                status_parts.append("⚠️ 0 LoRA layers detected!")
            
            if trigger_tag:
                status_parts.append(f"🏷️ Trigger: '{trigger_tag}'")
            
            logger.info(f"LoRA adapter loaded: layers={num_lora_layers}, params={num_lora_params}, trigger='{trigger_tag}'")
    ...
    def unload_lora(self) -> str:
        ...
        try:
            import copy
            import gc as _gc
            # Keep reference to old LoRA-wrapped decoder so we can free it
            old_decoder = self.model.decoder

            # Restore base decoder from CPU backup → GPU
            self.model.decoder = copy.deepcopy(self._base_decoder)
            self.model.decoder = self.model.decoder.to(self.device).to(self.dtype)
            self.model.decoder.eval()

            # Free the old LoRA-wrapped decoder from GPU
            ...
            # Free the CPU backup too — no longer needed
            del self._base_decoder
            ...
            self._current_lora_metadata = {}
            
            logger.info("LoRA unloaded, base decoder restored, VRAM freed")
    ...
    def get_lora_status(self) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with LoRA status info including trigger tag
        """
        status = {
            "loaded": self.lora_loaded,
            "active": self.use_lora,
            "scale": self.lora_scale,
            "path": self._current_lora_path or "",
            "trigger_tag": self._current_lora_metadata.get("trigger_tag", ""),
            "tag_position": self._current_lora_metadata.get("tag_position", "prepend"),
        }
        if self.lora_loaded:
            status["name"] = os.path.basename(self._current_lora_path) if self._current_lora_path else ""
            status["rank"] = self._current_lora_metadata.get("rank", 0)
            status["alpha"] = self._current_lora_metadata.get("alpha", 0)
            status["target_modules"] = self._current_lora_metadata.get("target_modules", [])
        return status
    ...
    def initialize_service(
        ...
        try:
            ...
            # Clean up any stale LoRA backup before reinitializing (prevents double-model memory)
            if self._base_decoder is not None:
                ...
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
                logger.info("[initialize_service] Cleared stale LoRA backup before model switch")
