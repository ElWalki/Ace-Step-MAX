...
class GenerationParams:
    ...
    # Measure alignment
    align_to_measures: bool = False  # Snap duration to complete musical measures
...
def _align_duration_to_measures(
    duration: float,
    bpm: Optional[int],
    time_signature: Optional[str],
) -> float:
    """Snap audio duration to the nearest complete musical measure boundary.
    
    A measure's length in seconds = (beats_per_measure / bpm) * 60
    
    For example:
    - 4/4 at 120 BPM: 1 measure = 2.0s, so 47s → 48s (24 measures)
    - 3/4 at 90 BPM:  1 measure = 2.0s, so 47s → 48s (24 measures)
    - 4/4 at 140 BPM: 1 measure ≈ 1.71s, so 47s → 47.14s (27.5→28 measures)
    
    Args:
        duration: Original duration in seconds
        bpm: Beats per minute (None defaults to 120)
        time_signature: Beats per measure as string (None defaults to "4")
    
    Returns:
        Duration rounded up to the nearest complete measure
    """
    # Defaults
    effective_bpm = int(bpm) if bpm and bpm > 0 else 120
    
    # Parse time signature: stored as "2", "3", "4", or "6"
    beats_per_measure = 4  # default 4/4
    if time_signature:
        try:
            ts_val = int(str(time_signature).strip())
            if ts_val in (2, 3, 4, 6):
                beats_per_measure = ts_val
        except (ValueError, TypeError):
            pass
    
    # Calculate measure duration in seconds
    measure_duration = (beats_per_measure / effective_bpm) * 60.0
    
    if measure_duration <= 0:
        return duration
    
    # Round up to complete measures
    num_measures = math.ceil(duration / measure_duration)
    aligned = num_measures * measure_duration
    
    # Clamp to valid range (10-600 seconds)
    aligned = max(10.0, min(600.0, aligned))
    
    # Round to nearest integer for clean values (model uses integer duration internally)
    aligned = round(aligned)
    
    logger.info(
        f"[measure_align] BPM={effective_bpm}, time_sig={beats_per_measure}/4, "
        f"measure={measure_duration:.2f}s, measures={num_measures}, "
        f"original={duration:.1f}s → aligned={aligned}s"
    )
...
def generate_music(
    ...
    try:
        ...
        # Measure alignment: snap duration to complete measures
        if params.align_to_measures and audio_duration and audio_duration > 0:
            audio_duration = _align_duration_to_measures(audio_duration, bpm, time_signature)
            logger.info(f"[generate_music] Measure-aligned duration: {audio_duration}s")
